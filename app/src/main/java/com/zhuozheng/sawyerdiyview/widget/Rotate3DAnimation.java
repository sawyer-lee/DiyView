...
/**在Android中如果想要实现3D效果一般有两种选择，一是使用Open GL ES，二是使用Camera。
 Open GL ES使用起来太过复杂，一般是用于比较高级的3D特效或游戏，像比较简单的一些3D效果，使用Camera就足够了。
 Camera中提供了三种旋转方法，分别是rotateX()、rotateY()和rotateZ，
 调用这三个方法，并传入相应的角度，就可以让视图围绕这三个轴进行旋转*/
